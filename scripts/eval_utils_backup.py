...
def extract_info(dataset, text):
    ...
    if dataset.lower() == "movielens1m":
        ...
        if match:
            # Return the extracted research interests
            return match.group(1).strip()
    elif dataset.lower() in {'edge_existence', 'connectivity', 'hamilton_path', 'cycle_checking', 'graph_structure_detection', 'graph_automorphic'}:
        return text
    else:
...
def get_best_trained_model(experiment, model_name):

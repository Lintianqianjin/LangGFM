...
# nohup python scripts/batch_eval_structure.py meta-llama/Llama-3.1-8B-Instruct > logs/llama_pipeline.log 2>&1 &

# Run Qwen2.5-7B-Instruct on GPU 1 in the background
nohup python scripts/batch_eval_structure.py Qwen/Qwen2.5-7B-Instruct > logs/qwen_pipeline.log 2>&1 &

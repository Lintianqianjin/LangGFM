...
def compute_grpo_outcome_advantage(token_level_rewards: torch.Tensor,
    ...
    scores = token_level_rewards.sum(dim=-1) # sum of kl and response reward shape (bs,)
    ...
    with torch.no_grad():
        bsz = scores.shape[0] # bs is not the same as the number of prompt, but the number of response
        for i in range(bsz):
            id2score[index[i]].append(scores[i]) #index[i] should be prompt index, one prompt is associated with multiple responses, which is group
        for idx in id2score:
            if len(id2score[idx]) == 1:
                id2mean[idx] = torch.tensor(0.0)
                id2std[idx] = torch.tensor(1.0)
            elif len(id2score[idx]) > 1:
                id2mean[idx] = torch.mean(torch.tensor(id2score[idx])) # group mean
                id2std[idx] = torch.std(torch.tensor([id2score[idx]])) # group std
            else:
